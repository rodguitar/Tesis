...
Sub EditaReview(titulo As String, id_review As String)
	Try
		...
		Log("OK UPDATE REVIEW SET titulo Realizado.")
		
	Catch
		Log("catch UPDATE REVIEW SET titulo   " & LastException.Message)
...
Sub EliminaReview(id_review As String)
	...
	Try
		...
		Log("OK DELETE from REVIEW where id_revie Realizado.")
		
	Catch
		Log("catch DELETE from REVIEW where id_revie   " & LastException.Message)
